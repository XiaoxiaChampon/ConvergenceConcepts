...
#   xlab(paste0("Observed Mean Proportion for bin","(",nn,",",p,")"))+transition_states(n)
...
maxu <- as.data.frame(
  lapply(ns,
         FUN = function(x, M, nn, p){replicate(M, max(runif(n = x, 0, 1)))},
         M = M, 0, 1),
  col.names = paste0("n ",ns))
...
maxuplot<-ggplot(maxu_long, aes(x=maximum)) +
        ...
        ylab("Value") + 
        xlab("Observed Maximum Value of uniform (0, 1)")
ggsave("maxuplot.png")
...
From above, let $\epsilon= \frac{t}{n}$, then $P(X(n)≤1−\frac{t}{n})=(1−\frac{t}{n})^n \rightarrow e^{−t}$. 
